...
\begin{abstract}
Traffic accidents are a concerning issue in India. Over 150,000 fatalities are reported each year. In this paper, we propose an integrated pipeline that combines DBSCAN spatial clustering with Random Forest classification. The former monitors accident black spots, while the latter predicts accident severity. We leverage 12,316 Indian road accident records and 17 engineered features.DBSCAN with haversine distance and $\varepsilon = 0.045$ radians identifies 9 accident clusters. The Random Forest model, trained with 200 estimators and balanced class weighting, reaches 84.66\% test accuracy and a weighted F1-score of 0.7793. Five-fold stratified cross-validation gives a mean accuracy of 84.76\%. An ensemble of gradient-boosted trees predicts crashes using traffic volume, sociodemographic, and roadway infrastructure data. A separate gradient-boosted model predicts crash severity using crash, traffic volume, and sociodemographic data. An ARI combines the predictions and weights the severity using the cell’s average crash rate from the previous 5 years.
\end{abstract}
...
India accounts for roughly 11\% of global road accident fatalities, despite having only about 1\% of the world's registered vehicles \cite{b1}. The Ministry of Road Transport and Highways has reported 461,312 road accidents and 168,491 deaths in 2022, which essentially translates into about 462 fatalities per day \cite{b2}. These numbers have remained distressingly high for well over a decade, with estimates suggesting that the economic costs are between 3 and 5\% of India’s GDP per annum \cite{b3}.

The majority of existing methodologies for black spot identification assume that the magnitude of the observation error equals the expectation of the underlying Poisson motorcycle crash process. They are appropriate for a steadily growing population but are too sensitive for small increases in crashes. Other methods are based on estimation theory rather than spatial clustering theory and ignore right-censoring, which results in biased estimate for extensively right-censored data, such as the motorcycle crash data in India.
...
\item An index that combines severity, accident density, and environmental conditions into one score per cluster, with weights determined by the Random Forest's own feature importances rather than set arbitrarily.
\end{enumerate}
...
GIS and spatial statistics have been used for hotspot identification. Anderson \cite{b4} mapped crash concentrations with kernel density estimation, while Kumar and Toshniwal \cite{b6} showed that DBSCAN works good for Indian accident data because it does not require a pre-set number of clusters. Prasannakumar et al. \cite{b8} also reported that spatial clustering helps isolate the road segments carrying a disproportionate share of crashes.
...
There are three gaps motivating this work: cluster membership is not used inside the severity model most of the time, many of the risk indices are still dependent on manual weighting, and multi-city Indian studies are still limited. This paper aims to fill these gaps by feeding the DBSCAN cluster IDs into the Random Forest model and deriving the ARI weights from the importances of the model.
...
The dataset used in this study is the ``Road Accident Severity in India'' dataset, publicly available on Kaggle \cite{b19}. It has 12,316 accident records with 37 raw columns having accident circumstances, road conditions, vehicle information, and driver characteristics.The target variable we aim to predict is Accident\_severity. The variable is categorical and has three levels. Accidents are categorized based on the most serious human injury recorded: 1 represents a Slight Injury, 2 represents a Serious Injury, and 3 represents a Fatal Injury.

Table~\ref{tab:dataset_overview} presents the key properties of the dataset after preprocessing.
...
The severity distribution is heavily imbalanced, as shown in Table~\ref{tab:severity_dist}. Slight injuries represent 84.56\% of all records, while fatal injuries constitute only 1.28\%. This imbalance directly affects classifier training and evaluation, a point we return to in Section~V.
...
\textbf{Geocoding.} The original dataset does not have latitude and longitude fields. Instead, it has an Area\_accident\_occured column having values like ``Office areas'', ``Market areas'', and ``School areas''. These categories are then mapped to representative coordinates of major Indian cities, and we apply random spatial jitter of 2 to 8 km to simulate within-city variation.

\textbf{Temporal feature engineering.} The Time column is processed to extract the hour of day (0--23). The Day\_of\_week field is mapped to numeric values (0 = Monday through 6 = Sunday). A binary Is\_Night variable which takes the value 1 when lighting conditions indicate darkness is derived from Light\_conditions.



\textbf{Weather binning.} The Weather\_conditions column, which contains many fine-grained categories, is consolidated into six groups: Clear, Rain, Fog, Snow, Wind, and Other. This minimizes sparsity while preserving the most safety-relevant weather distinctions.
...
DBSCAN is an algorithm that groups together points that are close to each other and classifies the points that lie alone in low-density regions as outliers \cite{b20}. It does not require the number of clusters to be specified. We implement the algorithm using the haversine distance and radian-transformed coordinates as input, and we use ball-tree indexing to accelerate the neighbor search.

DBSCAN's behavior is controlled by two parameters:
...
\item \textbf{MinPts}: the minimum number of points needed to define a dense area. Set to 15.
\end{itemize}
...
After each iteration, an updated centroid is computed by taking the average of all incidents classified in the respective cluster. The algorithm stops when the centroids stabilise (i.e. do not change between iterations) or when reaching the defined maximum iterations.

\subsection{Random Forest classification}

Random Forest is an ensemble of decision trees that are trained on bootstrapped subsets of data \cite{b21}. This classifier leverages the 17 features presented in Table~\ref{tab:features} in order to predict three severity classes. An 80/20 stratified split divides the data into 9,852 training samples and 2,464 test samples.
...
The ARI generates a single composite score for each cluster, incorporating three key factors: severity, accident density, and environmental conditions. Its formulation is as follows:
...
\item $S_{dens}$ is the score of the density, which is calculated for each cluster by dividing the incident count of it to the maximum incident count among all clusters.
\item $S_{env}$ is the score of the environment, which is mapped from the dominant weather condition of each cluster based on the mapping shown in Table~\ref{tab:env_scores}.
\end{itemize}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/fig1_severity_distribution.png}}
...
\textbf{Temporal patterns.} Fig.~\ref{fig:hourly} displays a bimodal pattern with peaks during morning and evening commute hours. Fig.~\ref{fig:daily} presents that Friday has the highest accident count, which matches earlier findings on end-of-week risk \cite{b11}.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/fig10_hourly_distribution.png}}
\caption{Accident frequency by hour of day. Peaks coincide with morning and evening commuting periods.}
\label{fig:hourly}
\end{figure}

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/fig11_daily_distribution.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/fig2_severity_weather_heatmap.png}}
...
DBSCAN found 9 clusters from the 12,316 geocoded records. It detected few noise points, mainly because the epsilon (287~km) is large enough to classify all but the sparsest indications as a cluster.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/fig3_dbscan_clusters.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=0.85\columnwidth]{figures/fig4_confusion_matrix.png}}
...
The macro F1-score of 0.3149, Cohen's Kappa of 0.0198, and MCC of 0.0749 indicate that the model does not separate the minority classes well, even though the headline accuracy looks strong.

\textbf{ROC curves.} Fig.~\ref{fig:roc} shows one-vs-rest ROC curves with AUC values of 0.6918 for Slight, 0.6981 for Serious, and 0.8266 for Fatal. The relatively high Fatal AUC, despite zero recall, suggests that the model assigns somewhat higher fatal probabilities to those cases even though the final class decision never chooses them.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/fig6_roc_curves.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/fig5_feature_importances.png}}
...
\textbf{Learning curve.} Fig.~\ref{fig:learning} shows a clear case of overfitting: training accuracy reaches 1.0 while validation accuracy plateaus near 0.85.

\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/fig7_learning_curve.png}}
...
To place the Random Forest results in context, five additional classifiers were trained on the same split and feature set. Table~\ref{tab:model_comparison} shows that the Random Forest model has the best accuracy. In contrast, the Decision Tree model achieves the highest macro F1-score, which indicates that its predictions are slightly more balanced.
...
All models indicate low Cohen's Kappa values indicating weak separation of the minority class, with features available to better capture temporal and spatial patterns as opposed to crash dynamics.
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/fig8_risk_map.png}}
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{figures/fig13_cluster_severity.png}}
...
The system has a clear strength: DBSCAN finds geographically coherent clusters without pre-defining the number of regions, and the ARI turns those outputs into a priority list that is easier to act on than raw counts alone. Including Cluster\_ID in the classifier also adds measurable spatial context.

The main limitation of this approach is class imbalance. With 84.56\% of records belonging to the Slight Injury class, the classifier will already tend to select Slight for most inputs. Balanced class weighting helps to alleviate this but, on its own, is not sufficient. This limitation is exacerbated by several other factors:
...
\item The dataset includes only 158 records of fatal injuries, which is a very small 1.28\% of the total. This small number makes it difficult for a tree-based ensemble model to find consistent patterns, even if class weighting is used.
...
Despite that shortcoming, the solution gives access to a full pipeline from raw data to a risk-tiered map, and the Flask API ensures the workflow is accessible from external applications.

\section{Conclusion and Future Work}

This paper presented an integrated accident hotspot prediction system that combines DBSCAN spatial clustering, Random Forest classification, and a data-driven ARI.Using 12,316 Indian road accident records, DBSCAN found nine clusters. The Random Forest model then achieved a test accuracy of 84.66\%. The hour of the day, the cause of the accident, and the day of the week were the most important factors in predicting accident outcomes.The ARI then separated the clusters into two Severe, two Moderate, and five Low-risk groups.
...
\item \textbf{Richer spatial data.} Substituting geocoded area labels with real GPS coordinates would most probably enhance clustering quality and Cluster\_ID utility.

\item \textbf{Alternative models.}Models that incorporate spatial structure might be better at capturing interactions that tree-based methods could miss.

\item \textbf{Real-time integration.} Live traffic and weather feeds could transform the current workflow into an early-warning system.
